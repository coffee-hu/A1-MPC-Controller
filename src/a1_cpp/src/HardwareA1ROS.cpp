    ...
    : safe(UNITREE_LEGGED_SDK::LeggedType::A1), udp(UNITREE_LEGGED_SDK::LOWLEVEL)
{
    ...
    // 初始化关节和脚的顺序，非常重要，因为不同系统会使用不同的关节编号顺序，而这个映射可以帮助统一不同系统的顺序
...
bool HardwareA1ROS::main_update(double t, double dt)
{
    ...
    joy_cmd_body_height += joy_cmd_velz * dt; // 机器人目标身体高度
...
void HardwareA1ROS::receive_low_state()
{
    ...
    while (destruct == false)
    {
        ...
        // foot force, add a filter here 通过滑动窗口滤波对四条腿的力传感器进行滤波，但是我们的机器人没有力传感器
        for (int i = 0; i < NUM_LEG; ++i)
        {
        ...
        // 使用前向运动学和雅可比矩阵计算每条腿的位置和速度，将这些位置和速度转换到世界坐标系下
        for (int i = 0; i < NUM_LEG; ++i)
        {
            // 计算脚的位置
